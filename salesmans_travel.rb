def travel(r, zipcode)
  raw_str = r.split(',').find_all { |x| x.scan(/[A-Z]{2} \d{5}/).first == zipcode}
  streets = raw_str.join(' ').scan(/[a-zA-Z _.]+[A-Z]{2}/).map { |x| x[1..-4]}.join(',')
  house = raw_str.each_with_object([]) { |x, arr| arr << x.match(/^\d+/) }
  "#{zipcode}:#{streets}/#{house.join(',')}"
end
